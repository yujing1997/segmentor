...
# For-loop version of nuclei_classify_extract_job_arrays_narval.sh but batch paralell processing 
...
# Define batch size and number of workers based on available resources
BATCH_SIZE=10  # Number of rows to process in each batch
NUM_WORKERS=32  # Increase for more parallelism if your CPU/RAM supports it
...
process_row() {
    ...
    # Run the Python script for nuclei classification
    start_time=$(date +%s)
    ...
    echo "Completed nuclei classification for WSI: $FILE_NAME in $((end_time - start_time)) seconds." | tee -a "$LOG_FILE"
}

# Process each row in batches for improved parallelism
for ((i = 0; i < ${#lines[@]}; i += BATCH_SIZE)); do
    batch=("${lines[@]:i:BATCH_SIZE}")

    # Process each line in the batch in the background
    for ((j = 0; j < ${#batch[@]}; j++)); do
        line="${batch[j]}"
        process_row $((i + j + 2)) "$line" &
    done

    # Wait for all processes in the current batch to complete before starting the next batch
    wait
done
...
# Below is a pure for-loop version of the nuclei_classify_extract_job_arrays_narval.sh script.
# But it doesn't take up the CPU Ram (128Gb) sufficiently, so need to parallel process batch (above script)
